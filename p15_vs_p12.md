...
- **P15 (Dual-Mode BLE + Classic):** Advertises both Bluetooth Classic (BR/EDR) and BLE. USB port is primarily for charging.
- **P12 (BLE + Native USB Printer Class):** Pure BLE peripheral with full native USB Printer Class device support (`0x09C7:0x0011`, `usblp`, `bInterfaceClass=0x7`). Communicates bidirectionally over `/dev/usb/lp*` and PyUSB bulk endpoints (`0x01` OUT / `0x81` IN).

### 3.5 USB Commands & Queries
...
- **Thickness / Burn Control:** `10 FF 10 00 00` -> responds `4F 4B` (`OK`)
- **Paper Feed Modes:**
